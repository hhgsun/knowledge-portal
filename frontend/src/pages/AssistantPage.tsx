...
import { Bot, Send, Sparkles, Search, BarChart3, MessageCircle, FileText, ShieldCheck, AlertTriangle, Loader2, ExternalLink } from "lucide-react";
...
export default function AssistantPage() {
  ...
  const visibleModes = modes.filter(item => item.value !== "analytics" || user?.role === "admin" || user?.role === "editor");
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    try {
      const result = await fetchWithAuth("/api/assistant", {
        method: "POST",
        noRetry: true,
        body: JSON.stringify({ message: text, preferredRoute: mode }),
      });
      ...
      <form onSubmit={submit} className="rounded-2xl border border-zinc-200 bg-white p-4 shadow-sm dark:border-zinc-800 dark:bg-zinc-900">
        ...
        <div className="mt-3 flex flex-col gap-3 border-t border-zinc-100 pt-3 sm:flex-row sm:items-center sm:justify-between dark:border-zinc-800">
          ...
          <button type="submit" disabled={!message.trim() || loading}
            ...
            {loading ? "Yönlendiriliyor" : "Gönder"}
          </button>
        </div>
      </form>

      {response && <AssistantResult response={response} />}
    </div>
  );
}
...
function AssistantResult({ response }: { response: AssistantResponse }) {
  return <section className="space-y-4" aria-live="polite">
    <div className="flex flex-wrap items-center gap-2 text-xs">
      ...
      <span className="text-zinc-500">Güven %{Math.round(response.confidence * 100)}</span>
    ...
    {(response.answer || response.clarification) && <div className="rounded-2xl border border-zinc-200 bg-white p-5 dark:border-zinc-800 dark:bg-zinc-900">
      ...
      <div className="prose prose-sm max-w-none text-zinc-700 dark:prose-invert dark:text-zinc-300">
        <ReactMarkdown remarkPlugins={[remarkGfm]}>{response.answer ?? response.clarification ?? ""}</ReactMarkdown>
      </div>
    ...
    {response.rag && <RagSources rag={response.rag} />}
    ...
    {response.results.length > 0 && <div className="rounded-2xl border border-zinc-200 bg-white dark:border-zinc-800 dark:bg-zinc-900">
      ...
      <div className="divide-y divide-zinc-100 dark:divide-zinc-800">{response.results.map(result =>
        <Link key={result.id} to={`/articles/${result.slug}`} className="flex gap-3 px-5 py-3 transition-colors hover:bg-zinc-50 dark:hover:bg-zinc-800/50">
    ...
    {response.rag && response.rag.evidence.length > 0 && <details className="rounded-2xl border border-zinc-200 bg-white dark:border-zinc-800 dark:bg-zinc-900">
      ...
      <div className="space-y-3 border-t border-zinc-100 p-5 dark:border-zinc-800">{response.rag.evidence.map((item, index) =>
        <div key={`${item.sourceId}-${item.chunkId ?? index}`} className="border-l-2 border-blue-400 pl-3 text-xs">
          <div className="font-medium text-zinc-700 dark:text-zinc-300">{item.sourceId} · {item.sourceName || item.title}{item.pageNumber ? ` · sayfa ${item.pageNumber}` : ""}</div>
...
function RagSources({ rag }: { rag: NonNullable<AssistantResponse["rag"]> }) {
