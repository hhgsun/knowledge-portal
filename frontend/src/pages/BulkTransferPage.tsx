...
export default function BulkTransferPage() {
  ...
  return <div className="max-w-4xl mx-auto space-y-6">
    ...
    <section className="p-5 border border-zinc-200 dark:border-zinc-800 rounded-xl space-y-4">
      ...
      <LabeledDropdown label="When a matching article exists" value={policy} onChange={setPolicy} options={[{ value: "skip", label: "Skip existing" }, { value: "update", label: "Update existing" }, { value: "duplicate", label: "Create a copy" }]} />
    ...
    <section className="p-5 border border-zinc-200 dark:border-zinc-800 rounded-xl space-y-4">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        <LabeledDropdown label="Status" value={status} onChange={setStatus} clearable emptySelectionLabel="All statuses" options={(schema?.statuses ?? []).map((value) => ({ value, label: value }))} />
        {(schema?.classifications ?? []).map(category => (
          <div key={category.key} className="text-sm">
            ...
            <DropdownSelector
              ...
              clearable={category.cardinality === "single"}
              emptySelectionLabel={`Tüm ${category.label.toLocaleLowerCase("tr-TR")} değerleri`}
              onChange={values => setFacetFilters(previous => ({ ...previous, [category.key]: values }))}
            />
          </div>
        ))}
        <LabeledDropdown label="Author" value={authorId} onChange={setAuthorId} disabled={mine} clearable emptySelectionLabel="All authors" options={authors.map((author) => ({ value: author.id, label: author.name }))} />
        <LabeledDropdown label="Tag" value={tag} onChange={setTag} clearable emptySelectionLabel="All tags" options={tags.map((item) => ({ value: item.slug, label: item.name }))} />
...
function LabeledDropdown({ label, value, onChange, options, disabled = false, clearable = false, emptySelectionLabel }: { label: string; value: string; onChange: (value: string) => void; options: DropdownOption[]; disabled?: boolean; clearable?: boolean; emptySelectionLabel?: string }) { return <div className="text-sm"><span className="block text-xs font-medium text-zinc-500 mb-1">{label}</span><DropdownSelector label={label} options={options} selected={value ? [value] : []} onChange={values => onChange(values[0] ?? "")} disabled={disabled} clearable={clearable} emptySelectionLabel={emptySelectionLabel} /></div>; }
