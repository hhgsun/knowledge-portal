...
type Metrics = { recallAtK: number; mrr: number; ndcgAtK: number; factCoverage: number; citationCoverage: number; refusalAccuracy: number; forbiddenFactPassRate: number; p50LatencyMs: number; p95LatencyMs: number; passed: boolean; failedGates: string[] };

const defaultThresholds = { recallAtK: .8, mrr: .75, ndcgAtK: .75, factCoverage: .7, citationCoverage: .8, refusalAccuracy: .9, forbiddenFactPassRate: 1, p95LatencyMs: 30000 };
...
export default function RagEvaluationsPage() {
  ...
  const load = useCallback(async () => {
    const [d, r] = await Promise.all([fetchWithAuth("/api/admin/rag-evaluations/datasets"), fetchWithAuth("/api/admin/rag-evaluations/runs")]);
