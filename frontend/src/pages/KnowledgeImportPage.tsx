...
export default function KnowledgeImportPage() {
  ...
  const commit = async () => {
    ...
    try {
      const body = new FormData(); files.forEach(file => body.append("files", file));
      body.append("manifest", JSON.stringify({ drafts: drafts.map(({ sourceIndex, title, content, excerpt, contentType, status, tags, keepOriginal }) => ({ sourceIndex, title: title.trim(), content, excerpt: excerpt?.trim() || undefined, contentType, status, tags, keepOriginal })) }));
  ...
  return <div className="max-w-7xl mx-auto">
    ...
    <div className="grid lg:grid-cols-[260px_minmax(0,1fr)] gap-6">
      ...
      {current && <section className="min-w-0 space-y-4">
        ...
        <input value={current.title} onChange={event => update({ title: event.target.value })} placeholder="Article title..." className="w-full text-2xl font-bold bg-transparent border-none outline-none placeholder:text-zinc-300 dark:placeholder:text-zinc-600"/>
        <input value={current.excerpt ?? ""} onChange={event => update({ excerpt: event.target.value })} placeholder="Brief description (optional)..." className="w-full text-sm bg-transparent border-none outline-none placeholder:text-zinc-400 text-zinc-600 dark:text-zinc-400"/>
        <div className="flex flex-wrap gap-3 pb-4 border-b border-zinc-200 dark:border-zinc-800"><select value={current.contentType} onChange={event => update({ contentType: event.target.value })} className="px-3 py-1.5 text-sm border border-zinc-300 dark:border-zinc-700 rounded-lg bg-white dark:bg-zinc-800">{contentTypes.map(type => <option key={type.value} value={type.value}>{type.label}</option>)}</select><select value={current.status} onChange={event => update({ status: event.target.value })} className="px-3 py-1.5 text-sm border border-zinc-300 dark:border-zinc-700 rounded-lg bg-white dark:bg-zinc-800"><option value="draft">Draft</option><option value="pending">Pending Review</option>{!isViewer && <option value="published">Published</option>}</select></div>
        ...
        {current.parsed && <Suspense fallback={<div className="h-64 bg-zinc-50 dark:bg-zinc-900 rounded-lg animate-pulse"/>}><TiptapEditor content={current.content} onChange={content => update({ content })}/></Suspense>}
