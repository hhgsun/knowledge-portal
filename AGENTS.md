...
- **Opt-in agentic retrieval pilot**: REST Assistant offers `retrievalStrategy=baseline|agentic`; baseline remains the default and MCP remains baseline-only. With `Assistant:AgenticRetrieval:Enabled=true`, `AgenticRetrievalPlanner` may produce up to four schema-constrained search queries before retrieval. The server retains the original question, rejects planner-proposed scope syntax/SQL/URLs, retains every user-supplied tag/author/classification filter, and runs each query through the existing published-only hybrid retrieval and grounding pipeline. Planning is time-bounded and fail-open to baseline; it has no data access, tool execution, write, general-chat, analytics, or free-form SQL capability. Strategy participates in answer-cache identity and is returned/audited through the Assistant response/tool call.
...
| Opt-in Agentic Retrieval | ✅ Implemented | REST/UI pilot alongside baseline: a schema-bounded, fail-open planner proposes up to four retrieval queries; original query, published-only scope, explicit tag/author/facet filters and all grounding checks remain mandatory. Disabled by default. |
