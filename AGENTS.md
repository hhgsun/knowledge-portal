...
- **Search RAG**: Chunk-level retrieval uses provenance-bearing article/attachment chunks. Narrow questions use `RagCandidateLimit` and a small distinct-source cap; broad intents use `RagBroadCandidateLimit` and map-reduce over the wider pool. `IRagContextBuilder` performs exact-duplicate suppression, source diversity, word budgeting, prompt hardening, and evidence-ID preservation; the chunks passed to grounding contain the exact bounded passages supplied to the LLM. Attachment source names appear in prompt citations. Evidence returns request-local source ID plus stable chunk ID, authenticated canonical article URL, and parsed PDF page number when available. Filters are applied inside vector retrieval and rechecked against article metadata.
- **RAG feedback**: The search UI records helpful/not-helpful against the authenticated caller's own RAG `searchQueryId`. Search metadata stores trace/prompt/index-profile/grounding versions and only a SHA-256 answer fingerprint, not a duplicate answer body; optional reason codes are bounded server-side.
