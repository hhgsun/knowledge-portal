...
- **Dynamic chat model selection**: Chat-capable models are discovered from `Ollama:BaseUrl` through `GET /api/tags` and, when available, verified through `POST /api/show`; the configured embedding model and embedding-only model names are excluded. The catalog is cached with last-known-good/configured fallback behavior. A session admin persists the default in `system_settings`; each user may persist `users.preferred_llm_model`. Effective precedence is available user preference → available admin default → `Ollama:ChatModel`/first discovered model. This affects Assistant/MCP chat generation and cache identity, not the fixed embedding or attachment-vision profiles.
...
| `profile.preferredLlmModel` | — | — | Optional chat model currently discovered from Ollama; clearing it restores the current admin default |
