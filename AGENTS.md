...
- **RAG answer profiles and provider budget**: Assistant `answerProfile` and MCP `answer_profile` accept `compact|balanced|comprehensive`; the UI keeps the selection in browser storage and responses expose the effective profile. Omitted broad/detailed requests may auto-promote from the configured `balanced` default to `comprehensive`. Comprehensive mode uses map-reduce and an evidence-capacity-bounded completeness gate; compact mode stays narrow. The profile participates in semantic-cache identity. Generation uses one canonical claim-only JSON contract, then renders visible Markdown from grounded claims. Every Ollama chat call receives explicit `num_ctx=Ollama:RagModelContextTokens`; defaults are a 32,768 model context, 4,096 output tokens and eight comprehensive claims when enough distinct relevant evidence facts exist.
- **Opt-in agentic retrieval pilot**: REST Assistant offers `retrievalStrategy=baseline|agentic`; baseline remains the default and MCP remains baseline-only. With `Assistant:AgenticRetrieval:Enabled=true`, `AgenticRetrievalPlanner` produces a schema-constrained research plan: bounded task/presentation, up to four search queries, natural-language scope candidates and a comprehensive-research flag. The server retains the original question; rejects planner-proposed scope syntax/SQL/URLs; resolves candidates only against portal metadata; and never treats model output as an authorization decision. Exact normalized tag matches take priority over active lookup value matches. Explicit user tag/author/classification filters are retained; inferred scope may only narrow them. Every query runs through the same published-only hybrid retrieval and grounding pipeline; a comprehensive plan takes the bounded map-reduce path. Planning is time-bounded and fail-open to deterministic baseline task/query behavior; it has no data access, tool execution, write, general-chat, analytics, or free-form SQL capability. Strategy participates in answer-cache identity and is returned/audited through the Assistant response/tool call.
...
│                         # Assistant: AssistantTurnPlanningService, AssistantPresentationService, AssistantOrchestratorService (isolated, read-only, task-aware composition over existing services)
...
| `assistant.answerProfile` / MCP `ask_knowledge.answer_profile` | — | — | Optional `compact|balanced|comprehensive`; defaults to balanced with automatic comprehensive promotion for broad/detailed intent. |
...
| Knowledge Assistant (RAG) | ✅ Implemented | Dedicated AI/RAG surface at `/api/assistant` and MCP `ask_knowledge`: bounded task-aware turn planning (`retrieve|transform_previous|clarify`), persisted grounded turn state, presentation-only follow-up transforms without retrieval, multi-turn standalone-query contextualization with retrieval-only HyDE, deterministic query understanding, hybrid/RRF evidence retrieval, production-bounded optional cross-encoder reranking, dynamic authority/freshness ranking, adaptive source breadth, calibrated token budgeting, per-response aggregated input/output token usage display (provider counts with explicit estimation fallback; cache hits and deterministic transforms report zero generation tokens), ACL-safe child→parent context, typed grounded synthesis and safe `paragraph|bullet_list|ordered_list|table|process_flow|infographic` content blocks, cited/consulted separation, conflict screening, fail-closed map-reduce, verified SSE with ordered real pipeline progress stages, one active owned session conversation per user (a new session permanently replaces the previous conversation), feedback, and principal/corpus/runtime-scoped semantic answer cache. It exposes no document-search, analytics, general-chat, write, or free-form SQL route. |
| RAG Answer Profiles | ✅ Implemented | Compact/balanced/comprehensive REST, MCP and UI profiles; automatic broad-intent promotion, claim-only structured generation, explicit Ollama context window and evidence-capacity-bounded comprehensive coverage. |
| Opt-in Agentic Retrieval | ✅ Implemented | REST/UI pilot alongside baseline: a schema-bounded, fail-open LLM research plan selects bounded task/presentation, up to four retrieval queries, portal-validated natural-language scope candidates and comprehensive retrieval when needed. Tag matches take priority over lookup values; original query, published-only scope, explicit tag/author/facet filters and all grounding checks remain mandatory. |
| Assistant Follow-up Topic Guard | ✅ Implemented | Subjectless follow-ups such as “nasıl kullanılır?” inherit the previous user topic; model rewrites that drop it are replaced by a deterministic standalone query and their HyDE hint is discarded. Assistant validation warnings are collapsed behind a count disclosure in the UI. |
| RAG Topic Alignment Gate | ✅ Implemented | Grounded claims must also match distinctive query-topic tokens through claim text or cited source title; explicit policy questions require a policy anchor. Same-evidence continuation facts remain valid, while short heading/catalogue labels cannot populate extractive fallback. Missing subjects fail closed instead of being filled from generic security/RBAC content. |
...
- **Assistant RAG**: For an owned conversation, `AssistantTurnPlanningService` first separates original request, retrieval query, task intent and presentation. A presentation-only command such as `sırala`, `maddele`, `tablo yap`, `özetle`, `akış şeması yap` or `infografik yap` deterministically re-renders only the previous validated claims and citations, without retrieval or LLM generation; without prior grounded state it asks a clarification question. Knowledge follow-ups continue through `AssistantQueryContextualizer`, which uses bounded recent user/assistant turns to rewrite an anaphoric or subjectless follow-up—including `nasıl entegre ederim?`—into a standalone query. It prefers the latest versioned grounded query as the topic anchor, so intervening presentation turns cannot replace the subject; legacy conversations fall back to the latest user turn. Malformed output or timeout falls back to a deterministic topic rewrite. Its optional hypothetical document is used only for a second dense lookup and can never become FTS input, evidence, or generation context. `RagQueryUnderstandingService` then extracts explicit metadata filters, expands configured synonyms/acronyms, and decomposes only compound questions up to `QueryUnderstanding:MaxQueries`; every subquery reuses the same effective ACL filter. `HybridRagRetriever` fuses lexical/vector child ranks, then adds freshness, approval and generic lookup authority signals. After published/filter recheck, strong child hits resolve through their persisted FK to one deduplicated structure-bounded parent; parent lookup is constrained to the authorized article set. Legacy parentless rows use bounded same-section neighbors only during rolling re-index. `IRagContextBuilder` performs exact-duplicate suppression, source diversity, calibrated token budgeting, prompt hardening, and evidence-ID preservation. `AssistantPresentationService` renders allowlisted typed blocks from those claims; raw model HTML/SVG or arbitrary visualization code is never executed.
- **RAG answer trust signals**: Assistant responses warn when cited sources are overdue, due for review, lack a recorded review date, or are not approved. Deterministic conflict screening also flags competing numeric, polarity, required/optional, and allowed/forbidden policy statements. The UI presents grounding states in plain Turkish and separates directly cited sources from other consulted sources.
...
- **MCP capability split**: `search_articles` mirrors the `fulltext|semantic|hybrid`, topic/author/classification filters, content/attachment inclusion, indexing state, warnings, search recording, and document result shapes of `GET /api/search`, but deliberately omits `onlyOwnContent` and always searches the complete published corpus. `ask_knowledge` mirrors the grounded Assistant execution boundary over that same published corpus and returns answer, claims, cited/consulted sources, evidence, grounding metadata, and trace information without creating a search record. The two MCP AI lanes have separate concurrency slots and circuit state.
