...
- **Pattern**: Controllers → Services → EF Core DbContext → PostgreSQL. Shared domain logic lives in `Services/`; `ArticleMutationService` owns article write invariants, `ContentTypeService` owns active content-type resolution, `KnowledgeQueryScopeService` owns shared inline/explicit scope parsing, `SearchExecutionService` owns document retrieval, and `KnowledgeAnswerService` owns grounded RAG generation for Assistant/MCP. Search never generates an answer; Assistant never returns a search-result or analytics route. Controllers keep only routing, auth scoping, and response shaping. **No duplicated logic across controllers**; if two endpoints need the same behavior, extract it into a service (or a `Helpers/` static for pure functions). Service failures return `ServiceError` (mapped via `ToActionResult()`).
...
| Knowledge Assistant (RAG) | ✅ Implemented | Dedicated AI/RAG surface at `/api/assistant` and MCP `ask_knowledge`: deterministic query understanding, hybrid/RRF evidence retrieval, dynamic authority/freshness ranking, adaptive source breadth, calibrated token budgeting, ACL-safe child→parent context, typed grounded synthesis, cited/consulted separation, conflict screening, fail-closed map-reduce, verified SSE, owned conversations, feedback, and principal/corpus/runtime-scoped semantic answer cache. It exposes no document-search, analytics, general-chat, write, or free-form SQL route. |
...
- **Search filters**: `GET /api/search` accepts optional query parameters: `onlyOwnContent` (boolean, API key auth only — filters to articles created by that API key), `includeContent` (boolean — includes canonical Markdown in `contentMarkdown`), `includeAttachments` (boolean — includes attachment metadata array per article), `tag` (repeatable, tag slugs), `author` (repeatable, user slugs), `contentType` (repeatable, content type values). Filters apply to `fulltext`, `semantic`, and `hybrid`. Tags from `#syntax` and `tag` are merged; authors from `@syntax` and `author` are merged; content types from `##syntax` and `contentType` are merged. If only filters are specified without text, Search returns browse results. Assistant accepts equivalent scope arrays but requires a question.
