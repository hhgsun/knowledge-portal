...
| Search (RAG) | ✅ Implemented | Deterministic query understanding (metadata filters, configurable synonym rewrite, selective decomposition), hybrid/RRF fusion, freshness/authority ranking, local or optional external reranking, parent-aware neighbor expansion, context diversity/deduplication, and fail-closed grounded generation with resilience/partial map-reduce controls |
