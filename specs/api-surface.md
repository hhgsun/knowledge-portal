...
Generic category definitions expose stable `key`, `label`, `single|multiple` cardinality, required/default behavior, active state, display order, and `none|filter` RAG behavior (default `filter`). Classification inputs use canonical lookup values. Article create/update accepts `classifications: { "department": ["finance"] }`; article summaries/details return the same canonical map. Repeatable REST `facet=department:finance`, inline `facet:department=finance`, Assistant `facets`, and MCP `scope.facets[]` apply OR within a category and AND across categories. Unknown/inactive category or value filters fail closed.
