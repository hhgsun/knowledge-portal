...
| `retrievalStrategy` | string | No | `baseline` (default) or the opt-in `agentic` pilot. Agentic mode uses a bounded model-generated research plan to choose task/presentation, up to four queries, candidate portal scope and whether broad research is needed. Candidate scope is server-validated and can only narrow the supplied scope; it cannot invoke tools, execute SQL, or access non-published content. It is rejected unless `Assistant:AgenticRetrieval:Enabled=true`. |
...
When `retrievalStrategy` is `agentic`, `AgenticRetrievalPlanner` first returns a schema-constrained research plan containing a bounded task/presentation, at most four short retrieval queries, short natural-language scope candidates and a broad-research flag. The server retains the original question; rejects scope syntax, SQL and URLs from planner output; and validates candidate terms against active portal metadata. Exact normalized tag matches take priority over lookup-value matches. Inferred scope can only add narrowing tag/facet conditions and can never remove explicit tag/author/facet filters. Each accepted query runs through the same hybrid retrieval and published-only recheck as baseline; broad plans use the bounded map-reduce path. Planning errors or timeouts fail open to deterministic baseline task/query behavior; answer generation remains claim/citation grounded. The strategy is part of semantic-cache identity and is returned in the response.
...
Generic category definitions expose stable `key`, `label`, `single|multiple` cardinality, required/default behavior, active state, editable display order, and `none|filter` RAG behavior (default `filter`). `PUT /api/lookups/categories` accepts `clearDefaultValue: true` to explicitly remove an optional category's default; this cannot be combined with `defaultValueId`, and required categories must retain a default. Values also expose editable display order. Classification inputs use canonical lookup values. Article create/update accepts `classifications: { "department": ["finance"] }`; article summaries/details return the same canonical map. Repeatable REST `facet=department:finance` is supported by article lists, bulk exports and Search; inline `+department:finance`, Assistant `facets`, and MCP `scope.facets[]` use the same OR-within/AND-across semantics. Unknown/inactive category or value filters fail closed.
