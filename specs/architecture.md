...
| **Assistant turn/presentation** | `AssistantTurnPlanningService`, `AssistantQueryContextualizer`, `AssistantPresentationService` | Separate original request, retrieval query, bounded task intent and presentation; choose `retrieve|transform_previous|clarify`; render only validated claims as allowlisted typed blocks |
| **RAG query/context** | `RagQueryUnderstandingService`, `AgenticRetrievalPlanner`, `ResearchScopeResolver`, `HybridRagRetriever`, `ExternalRagChunkReranker`, `RagContextExpansionService`, `IRagContextBuilder` | Baseline deterministic filter/decomposition or opt-in bounded LLM research plan (task, presentation, queries, scope candidates, comprehensive flag) → portal-validated tag-first scope narrowing → hybrid child retrieval → calibrated/bounded rerank → ACL-safe child→parent resolution → bounded evidence context. The planner cannot directly change scope or access data. |
...
9. **Strict Search/Assistant boundary** — `SearchExecutionService` returns documents only (`fulltext`, `semantic`, `hybrid`). `KnowledgeAnswerService` is the sole grounded-RAG pipeline for REST Assistant and MCP `ask_knowledge`; neither surface falls back into the other. `KnowledgeQueryScopeService` shares only filter semantics. `AssistantTurnPlanningService` is limited to knowledge retrieval, prior-grounded-answer transformation, and clarification; it does not route to analytics, mutations, general chat, or SQL. Owned conversations provide bounded follow-up context and persist a versioned grounded turn-state snapshot so presentation-only commands can reuse claims and citations without retrieval. The grounded semantic cache is isolated by principal scope, task/presentation, effective Ollama-discovered chat model, and corpus/runtime versions. `OllamaModelCatalogService` discovers and caches chat-capable `/api/tags` models with capability checks and stale/configured fallback; `LlmModelSelectionService` validates the Assistant request model and otherwise resolves the database-backed admin default → configured/first-discovered fallback. The Assistant browser choice lives only in browser storage; MCP and system jobs use the admin default. Embedding and attachment-vision models remain separate operational profiles.
...
- Query understanding validates canonical category/value pairs for inline `+category:value`, REST `facet=category:value`, JSON `facets`, and MCP `scope.facets[]` inputs. Typing `+` in the Search UI discovers active categories and typing `+category:` discovers that category's active values. Explicit facets are hard filters. General natural-language inference is not performed, preventing low-confidence classification from reducing recall.
...
- Ranking authority is centrally governed by `lookup_values.authority_weight` (0-100) for every dynamic lookup category. Hybrid and RAG rerankers use the highest weight among an article's active assignments in active categories; articles without generic assignments retain a legacy `content_type` lookup fallback. Per-content-type `Ollama:Ranking:Authority:*` configuration is not used. `Ollama:Ranking:AuthorityWeight` remains only the bounded contribution multiplier.
