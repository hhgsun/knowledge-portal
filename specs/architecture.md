...
| **RAG query/context** | `RagQueryUnderstandingService`, `HybridRagRetriever`, `ExternalRagChunkReranker`, `RagContextExpansionService`, `IRagContextBuilder` | Deterministic filter/decomposition → hybrid child retrieval → calibrated/bounded rerank → ACL-safe child→parent resolution → bounded evidence context |
