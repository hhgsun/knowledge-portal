...
| **RAG query/context** | `RagQueryUnderstandingService`, `AgenticRetrievalPlanner`, `HybridRagRetriever`, `ExternalRagChunkReranker`, `RagContextExpansionService`, `IRagContextBuilder` | Baseline deterministic filter/decomposition or opt-in bounded query planning → hybrid child retrieval → calibrated/bounded rerank → ACL-safe child→parent resolution → bounded evidence context. The planner returns queries only; it cannot change scope or access data directly. |
