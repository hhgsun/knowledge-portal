...
Conversation contextualization reads only an owned session's bounded recent user/assistant turns; the transcript is marked untrusted, explicit scope tokens are preserved, and timeout/malformed output fails open to a deterministic topic rewrite. The optional hypothetical document is never evidence: it is used only for a second ACL-filtered dense lookup and is excluded from FTS, source context, citations, and generation.

External reranking is disabled by default. Enabling `Reranking:External` sends only bounded, already ACL-filtered candidate passages to an explicitly configured HTTPS or loopback endpoint. Redirects are disabled; request/response size, timeout, retry, concurrency and circuit limits apply; incomplete/non-finite score sets are rejected and arbitrary logits are normalized before blending. The integration never sends caller authorization tokens, falls back locally on overload/timeout/error/invalid output, and must be covered by the organization's provider privacy agreement. Parent/neighbor expansion reuses only stored chunks from the same authorized article, source/attachment, embedding model and derived parent section; it runs after the published/filter safety recheck.
