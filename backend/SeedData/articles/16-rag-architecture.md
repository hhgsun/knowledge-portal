...
Provider yalnız tek canonical claim nesnesi (`claims` ve `insufficientContext`) üretir; ayrı bir serbest metin `answer` alanı ürettirilmez. Prompt içinde özgün kullanıcı isteği, çözülmüş knowledge sorusu, response task ve presentation ayrı alanlardır; retrieval query'nin kullanıcı görevini ezmesi önlenir. Backend doğrulanmış atomik claim'lerden kullanıcıya görünen Markdown yanıtı ve allowlist edilmiş `markdown|paragraph|bullet_list|ordered_list|table|process_flow|infographic` typed bloklarını kurar. Modelden HTML, SVG, serbest tablo veya çalıştırılabilir görselleştirme kodu istenmez. Bu, aynı cevabın iki kez üretilmesini kaldırır, çıktı bütçesini gerçek kapsama ayırır ve sunum dönüşümünün yeni olgu üretmesini engeller. Legacy/fake çıktılardaki `answer` alanı yalnız geriye uyumluluk için okunabilir; doğruluk kaynağı değildir.
...
Kaynak bloklarındaki approval, dinamik authority, review state, reliability ve update zamanı güvenilir server metadata'sıdır. Sayısal, açık olumlu/olumsuz veya aynı konu üzerindeki zorunlu/opsiyonel ve izinli/yasak politika ifadeleri çelişirse öncelik sırası onay → otorite → review state → reliability → update zamanıdır; bütün sinyaller eşitse sistem otomatik seçim yapmaz. Deterministik `conflictAssessment` bu sınırlı politika kiplerini de aday çelişki olarak işaretler, ancak genel semantic contradiction tespiti yaptığını söylemez. Cited kaynaklar overdue, due_soon, not_recorded veya onaysız ise cevap warnings alanında açıkça belirtilir. Model her rakip kaynağın olgusunu ayrı `conflict` claim'i olarak üretir, böylece her cümle kendi kaynağına karşı bağımsız doğrulanabilir.
...
Kullanıcıya görünen yanıt yalnız doğrulamayı geçen claim'lerden yeniden oluşturulur. Böylece düzgün görünen fakat dayanağı olmayan model metni cevap içine sızamaz. `sources` yalnız doğrulanmış claim'lerde gerçekten atıf yapılan makaleleri, `consultedSources` ise modele verilen bütün makaleleri taşır. İki liste authority, approval, review, reliability ve update metadata'sını içerir; arayüz kaynakları “Kullanılan kaynaklar” ve “İncelenen diğer kaynaklar” olarak açıklar. Grounding durumları kullanıcıya “Kaynaklarla doğrulandı”, “Kısmen doğrulandı” veya “Yeterli kaynak bulunamadı” şeklinde gösterilir; eski/onaysız veya çelişkili kaynak uyarıları da sade Türkçe ile sunulur. Yanıt ayrıca provenance-bearing `evidence`, citation ID coverage, claim support coverage, grounding durumu, conflict assessment, partial/refusal bilgisi ve uyarıları taşır. Assistant arayüzü çok sayıdaki teknik doğrulama uyarısını varsayılan olarak kapalı, adet gösteren bir uyarı satırında toplar; kullanıcı ayrıntıları açtığında liste sınırlı yükseklikte kaydırılabilir. Evidence, prompt içi `sourceId` yanında stabil `chunkId`, yetki kontrollü `canonicalUrl` ve PDF provenance'ı varsa `pageNumber` döndürür.
...
Yönetici arayüzündeki RAG evaluation alanı dinamik golden dataset'ler ve eşikler tanımlar. Çalıştırmalar dataset, config, model ve prompt snapshot'larını saklar; Recall, MRR, NDCG, fact/citation/grounding/refusal/safety/latency metrikleri üretir. Dataset vakaları isteğe bağlı `turns` dizisiyle çok turlu görevler tanımlar; her turdaki expected intent, presentation ve retrieval kararı `conversationTaskAccuracy` ile `retrievalDecisionAccuracy` kalite kapılarına girer. CI, gerçek PostgreSQL/pgvector fidelity kapısına ve canlı Ollama golden-dataset kalite kapısına ek olarak REST Assistant turn-planning kontrolünü ve modern `/mcp` `ask_knowledge` çağrısının canlı model, kanıt ve trace sözleşmesini zorunlu çalıştırır.
