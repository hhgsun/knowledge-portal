...
- **classifications:** Kontrollü ve dinamik sınıflandırma nesnesidir; örneğin `{ "department": ["finance"], "system": ["erp"] }`. Kategoriler yönetim ekranından tekli/çoklu, zorunlu/opsiyonel, varsayılanlı ve görünüm sıralı olarak tanımlanır; kategori ve değer `sortOrder` alanları arayüzde düzenlenebilir. Opsiyonel bir kategoride “Varsayılan yok” seçilerek mevcut varsayılan kaldırılabilir; zorunlu kategoriler varsayılan değerlerini korumalıdır. AI davranışı varsayılan olarak `filter`dır; yalnız metadata amaçlı kategoriler `none` seçilebilir. Yalnız aktif ve canonical değerler kabul edilir. Makale formları aktif kategorileri alan olarak gösterir; Arama ekranı ise bunları `+kategori:değer` yazılırken otomatik tamamlar. Aynı kategorideki değerler OR, farklı kategoriler AND mantığıyla aranır. `contentType` geriye uyumluluk için korunur ve generic `content_type` atamasıyla otomatik eşlenir.
- Yeni makale ve makale düzenleme formlarındaki sınıflandırmalar, makale listesi filtreleriyle aynı ortak seçiciyi kullanır. Tekli/çoklu ve zorunlu/opsiyonel kuralları korunur; seçenekler tanımlı renk ve ikonlarıyla gösterilir, çoklu seçimler renkli rozetlerle özetlenir ve 10'dan fazla aktif değerde arama alanı açılır.

Makale listesinde filtreler kompakt ve sürekli görünür bir araç çubuğunda toplanır. Durum, sınıflandırma, etiket, tarih aralığı ve “Makalelerim” kontrollerinin yanında sıralama ve sonuç sayısı da aynı alanda sunulur. Dinamik sınıflandırma filtreleri etiket seçiciyle tutarlı biçimde açılır panel, çoklu seçim ve seçim işareti sunar; aktif seçenek sayısı 10'u geçtiğinde panelde yerel arama alanı da gösterilir. Aktif filtreler tek tek kaldırılabilen rozetlerle özetlenir. Sınıflandırma seçenekleri ve rozetleri Tanım Değerleri ekranında belirlenen Lucide ikonunu ve hazır ya da özel HEX rengini kullanır.
