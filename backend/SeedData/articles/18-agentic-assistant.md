...
`retrievalStrategy` varsayılan olarak `baseline` değerindedir. Pilot için `Assistant:AgenticRetrieval:Enabled=true` açıldığında arayüzde `agentic` (Araştırmalı) seçeneği görünür. Bu mod, yanıt üretilmeden önce modelden şema kontrollü bir araştırma planı alır: görevi (`answer`, `summarize`, `compare`, `analyze`, `list`, `procedure`, `explain`), sunumu, en fazla dört kısa retrieval sorgusunu, kısa doğal dil kapsam adaylarını ve kapsamlı araştırma gereksinimini belirler. Sunucu özgün soruyu her zaman korur; SQL/URL/tool komutu ve scope sözdizimi içeren plan çıktısını reddeder; kapsam adaylarını yalnız aktif portal metadata'sında doğrular. Aynı ifade hem tag hem lookup değeriyle eşleşirse tag önceliklidir. Model kapsamı doğrudan değiştiremez: kullanıcıdan gelen tag/yazar/facet filtreleri korunur, doğrulanmış aday yalnız ek daraltma yapabilir. Plan bozuk veya zaman aşımındaysa görev ve sorgular deterministik baseline davranışına geri döner; her sorgu aynı published-only, hibrit retrieval ve grounding kontrollerinden geçer.

Örneğin `Finagoport için dokümanları getir ve özetle` isteğinde plan `summarize` görevini ve kapsamlı araştırmayı seçebilir; `finagoport` aktif bir tag ise sunucu o tag altındaki bütün published makaleleri aday havuzuna alır. Bu, sınırsız makaleyi tek prompt'a koymak değildir: aday chunk'lar adil ve sınırlı map-reduce gruplarında işlenir. Süre, token veya aday sınırı kapsamı daraltırsa yanıt partial/capped uyarısı taşır ve tamamı incelenmiş gibi davranmaz. `API key yetkilerini açıkla ve rollerle karşılaştır` gibi bir istek ise `compare` görevini, API key ve rol matrisi için ayrı retrieval sorgularını seçebilir; nihai kıyas yalnız kanıtlanmış claim'lerden oluşturulur.
